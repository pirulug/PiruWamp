...
import threading
from ..managers.mariadb_repository import MariaDBRepository
from ..core.config import config
from ..core.logger import logger
from .theme import apply_theme

class MariaDBManagerWindow(tk.Toplevel):
    def __init__(self, parent_app):
        super().__init__(parent_app.root)
        self.title("Gestor de Versiones de MariaDB")
        ...
        self.repository = MariaDBRepository()
    ...
    def _setup_ui(self):
        ...
        tk.Label(main_frame, text="Versiones de MariaDB Disponibles", font=("Arial", 12, "bold")).pack(pady=(0, 10))
    ...
    def refresh_list(self):
        ...
        active = config.active_mariadb_version
        ...
        self.tree.insert("", tk.END, values=("Default (mysql/)", default_status, default_action), tags=("Default", ""))
    ...
    def activate_version(self, name):
        version_to_set = None if name == "Default (mysql/)" else name
        config.set_mariadb_version(version_to_set)
        messagebox.showinfo("MariaDB", f"Versión {name} activada.\nSe actualizarán las configuraciones al iniciar MariaDB.")
        self.refresh_list()
