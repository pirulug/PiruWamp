...
import threading
from ..managers.phpmyadmin_repository import PHPMYAdminRepository
from ..core.config import config
from ..core.logger import logger
from .theme import apply_theme

class PHPMYAdminManagerWindow(tk.Toplevel):
    def __init__(self, parent_app):
        super().__init__(parent_app.root)
        self.title("Gestor de Versiones de phpMyAdmin")
        ...
        self.repository = PHPMYAdminRepository()
    ...
    def _setup_ui(self):
        ...
        tk.Label(main_frame, text="Versiones de phpMyAdmin Disponibles", font=("Arial", 12, "bold")).pack(pady=(0, 10))
    ...
    def refresh_list(self):
        ...
        active = config.active_phpmyadmin_version

        # Add "Default" version (from etc/phpMyAdmin)
        default_status = "Instalado"
        default_action = "Activar"
        if active is None:
            default_status = "ACTIVO ✅"
            default_action = "Ya es el activo"
        self.tree.insert("", tk.END, values=("Default (etc)", default_status, default_action), tags=("Default", ""))
    ...
    def start_download(self, name, url):
        ...
        self.progress_label.config(text=f"Descargando {name}...")
    ...
    def finish_download(self, success, name):
        ...
        if success:
            messagebox.showinfo("Éxito", f"{name} se ha instalado correctamente.")
            self.refresh_list()
        else:
            messagebox.showerror("Error", f"No se pudo instalar {name}. Revisa los logs.")

    def activate_version(self, name):
        version_to_set = None if name == "Default (etc)" else name
        config.set_phpmyadmin_version(version_to_set)
        messagebox.showinfo("phpMyAdmin", f"Versión {name} activada.\n(Reinicia Apache para aplicar)")
        self.refresh_list()
